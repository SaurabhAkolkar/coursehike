            ...
            <form class="la-vcourse__purchase-form" id="add_to_cart_form" name="add_to_cart_form" method="post" action="/add-to-cart">
              ...
              <div class="la-vcourse__purchase-actions d-flex flex-wrap align-items-center mt-8">
                ...
                <div class="la-vcourse__purchase-btn w-50 text-center">
                  <a class="btn la-btn__plain text--green" <?php if(Auth::check()): ?> onclick="$('#add_to_cart_form').submit()" <?php else: ?> data-toggle="modal" data-target="#locked_login_modal" <?php endif; ?>>ADD TO CART</a>
                </div>
              </div>
            </form>              
                        ...
                        <div class="la-lcreviews__wrapper ">
                          ...
                          <div class="la-lcreviews__content">
                            ...
                            <div class="la-lcreviews__edit-comment text-right">
                              <?php if(Auth::check() && Auth::user()->id == $review->user_id): ?><a class="text-sm la-rtng__review-edit"  role="button" onclick="editReview(<?php echo e($review->id); ?>, <?php echo e($review->course_id); ?>, '<?php echo e($review->review); ?>', <?php echo e($review->rating); ?>)">Edit</a><?php endif; ?>
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          <?php else: ?>
            ...
            <div class="row row-cols-md-2 row-cols-lg-3 la-anim__stagger-item--x ">
              <?php $__currentLoopData = $related_courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $related_course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                   <?php if (isset($component)) { $__componentOriginal541dd97498dd76400e36bb15ebc47d888e5f7706 = $component; } ?>
<?php $component = $__env->getContainer()->make(App\View\Components\Course::class, ['id' => $related_course->id,'img' => $related_course->preview_image,'course' => $related_course->title,'url' => $related_course->slug,'rating' => round($related_course->average_rating, 2),'creatorImg' => $related_course->user->user_img,'creatorName' => $related_course->user->fname,'creatorUrl' => $related_course->user->id,'learnerCount' => $related_course->learnerCount,'price' => $related_course->price,'bought' => $related_course->isPurchased(),'checkWishList' => $course->checkWishList,'checkCart' => $course->checkCart]); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php $component->withAttributes([]); ?>
<?php if (isset($__componentOriginal541dd97498dd76400e36bb15ebc47d888e5f7706)): ?>
